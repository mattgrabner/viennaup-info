...
- \`GET $VIENNAUP_API_BASE_URL/events\`
  ...
  Treat \`query\` as fuzzy discovery, not exact title matching.
...
- \`GET $VIENNAUP_API_BASE_URL/near\`
  ...
  \`radiusKm\`, \`date\`, \`limit\`, \`excludeSlug\`, and \`excludeUid\`.
...
3. Prefer \`/near\` for location questions. Use \`radiusKm=1.5\` for walking
   distance and \`radiusKm=5\` for a broader area. When the user asks for
   events close to a known event, pass that event's \`excludeSlug\` so the
   origin event does not come back as the top result.
4. For exact event lookups, use \`/event\` with \`slug\` if you have it;
   otherwise narrow down with \`/events?query=...\`, then call \`/event\`.
